...
function isStrongPassword(p) {
  if (!p || p.length < MIN_PASSWORD_LENGTH) return false;
  // على الأقل حرف ورقم
  if (!/[a-zA-Z\u0600-\u06FF]/.test(p)) return false;
...
async function initDB() {
  const stmts = [
    ...
    `CREATE TABLE IF NOT EXISTS posts (
      ...
      text TEXT, time INTEGER, deleted INTEGER DEFAULT 0, edited INTEGER DEFAULT 0
    )`,
    ...
    `CREATE TABLE IF NOT EXISTS notifications (
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL,
      type TEXT, text TEXT, from_user_id INTEGER, post_id INTEGER,
      url TEXT, time INTEGER, read INTEGER DEFAULT 0
    )`,
    `CREATE TABLE IF NOT EXISTS audit_log (
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER,
      action TEXT, ip TEXT, details TEXT, time INTEGER
    )`,
    `CREATE TABLE IF NOT EXISTS login_attempts (
      username TEXT PRIMARY KEY, count INTEGER DEFAULT 0,
      locked_until INTEGER DEFAULT 0, time INTEGER
    )`
  ];
...
async function auth(req, res, next) {
  ...
  const age = Date.now() - (t.created_at || 0);
  if (age > TOKEN_TTL_MS) {
...
app.post('/api/register', async function(req, res) {
  try {
    ...
    if (pass.length < MIN_PASSWORD_LENGTH) return res.status(400).json({ error: 'password_too_short' });
    if (!isStrongPassword(pass)) return res.status(400).json({ error: 'password_too_weak' });
    ...
    const now = Date.now();
    const r = await run('INSERT INTO users (username, display_name, password, color, bio, location, country, phone, verified, created_at, last_seen, username_changed_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',
...
app.post('/api/login', async function(req, res) {
  try {
    ...
    // فحص قفل الحساب
    const attempt = await q1('SELECT * FROM login_attempts WHERE username = ?', [key.toLowerCase()]);
    if (attempt && attempt.locked_until > Date.now()) {
      const minutes = Math.ceil((attempt.locked_until - Date.now()) / 60000);
      return res.status(429).json({ error: 'account_locked', minutes_left: minutes });
    }
    ...
    if (!user || !verifyPassword(pass, user.password)) {
      const newCount = (attempt ? attempt.count : 0) + 1;
      let lockedUntil = 0;
      if (newCount >= ACCOUNT_LOCK_AFTER) {
        lockedUntil = Date.now() + ACCOUNT_LOCK_MS;
        auditLog(user ? user.id : null, 'account_locked', req, key);
      }
      await run('INSERT OR REPLACE INTO login_attempts (username, count, locked_until, time) VALUES (?,?,?,?)',
        [key.toLowerCase(), newCount, lockedUntil, Date.now()]);
      auditLog(user ? user.id : null, 'login_failed', req, key);
      return res.status(401).json({ error: 'invalid_credentials' });
    }
    
    // نجاح
    await run('DELETE FROM login_attempts WHERE username = ?', [key.toLowerCase()]);
    const token = genToken();
    const now = Date.now();
    await run('INSERT INTO tokens (token, user_id, created_at) VALUES (?,?,?)', [token, user.id, now]);
    await run('UPDATE users SET last_seen = ? WHERE id = ?', [now, user.id]);
    auditLog(user.id, 'login', req, '');
    res.json({ token: token, user: publicUser(user) });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'server_error' });
...
app.post('/api/change-password', auth, async function(req, res) {
  ...
  if (newPass.length < MIN_PASSWORD_LENGTH) return res.status(400).json({ error: 'password_too_short' });
  if (!isStrongPassword(newPass)) return res.status(400).json({ error: 'password_too_weak' });
  if (!verifyPassword(oldPass, req.user.password)) {
    auditLog(req.user.id, 'password_change_failed', req, '');
  ...
  // مسح كل التوكنات ما عدا الحالي
  await run('DELETE FROM tokens WHERE user_id = ?', [req.user.id]);
  const newToken = genToken();
  await run('INSERT INTO tokens (token, user_id, created_at) VALUES (?,?,?)', [newToken, req.user.id, Date.now()]);
  auditLog(req.user.id, 'password_change', req, '');
...
app.delete('/api/me', auth, async function(req, res) {
  ...
  auditLog(uid, 'account_delete', req, '');
  res.json({ ok: true });
});
...
app.get('/api/notifications', auth, async function(req, res) {
  try {
    ...
    for (const n of rows) {
      ...
      out.push({
        id: n.id, type: n.type, text: n.text, url: n.url,
        time: n.time, read: !!n.read, post_id: n.post_id, from: fromUser
      });
    }
...
app.get('/api/posts', auth, async function(req, res) {
  ...
  for (const p of rows) {
    ...
    posts.push({
      id: p.id, text: decryptText(p.text), time: p.time,
      user: publicUser(u), likes: lk.c, comments: cm.c, liked: liked, edited: !!p.edited
    });
  }
...
io.on('connection', function(socket) {
  ...
  socket.on('auth', async function(data) {
    try {
      ...
      if (age > TOKEN_TTL_MS) {
        await run('DELETE FROM tokens WHERE token = ?', [token]);
        return socket.emit('auth-error', { error: 'token_expired' });
      }
...
setInterval(async function() {
  try {
    ...
    // حذف محاولات دخول قديمة
    await run('DELETE FROM login_attempts WHERE time < ?', [Date.now() - 24 * 3600 * 1000]);
    // حذف audit log قديم (30 يوم)
...
async function start() {
  ...
  server.listen(PORT, '0.0.0.0', function() {
    ...
    console.log('CORS locked to ' + ALLOWED_ORIGINS.length + ' origins');
